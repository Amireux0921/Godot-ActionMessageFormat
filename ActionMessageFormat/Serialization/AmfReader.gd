...
class_name AmfReader extends ActionMessageFormat
...
func ReadAmfPacket()->AMF_Packet:
	var Packet = AMF_Packet.new()
	Packet.Version = self.ReadInt16()
	Packet.Headers = self.ReadAmfHeaders()
	Packet.Bodys = self.ReadBodys()
	return Packet


func ReadAmfHeaders()->Array[AMF_Header]:
	
	var value:Array[AMF_Header]=[]
	
	var HeaderCount = self.ReadInt16()

	for i in HeaderCount:
		
		var Header:= AMF_Header.new()
...
func ReadBody()->AMF_Body:
	
	var Body:= AMF_Body.new()
...
func ReadBodys()->Array[AMF_Body]:
	
	var value:Array[AMF_Body]=[]
...
func ReadAmf0()->Variant:
	...
	match type:
		...
		Amf0Type.AMF0Type.OBJECT:
			return serialization.Normalize(self.ReadAmf0Object()) 
		...
		Amf0Type.AMF0Type.TYPED_OBJECT:
			return serialization.Normalize(self.ReadAmf0TypedObject())
...
func ReadAmf3():
	...
	match type:
		...
		Amf3Type.AMF3Type.OBJECT:
			return serialization.Normalize(self.ReadAmf3Object()) 
...
func ReadAmf3Date()->DateTimes:
	var reference = self.ReadAmf3UInt29()
	
	if (reference & 0x01) == 0x01:
		
		var milliseconds = self.ReadDouble()
		var value = DateTimes.new(milliseconds)
